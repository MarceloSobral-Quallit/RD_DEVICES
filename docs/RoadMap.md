...
- **GitHub — projeto sob versionamento:** `git init` para o repo **privado** `github.com/MarceloSobral-Quallit/RD_DEVICES` (branch `main`). `.gitignore` reforçado (`*.pfx`/`*.p12`/`*.pem`, `tools/certs/`, `*.db-shm`/`-wal`, `release/`, `.specstory/`, `PROJETO_ORIGINAL/`, `analise*.md`). Segredos e infra interna (senhas de loja em `COLETOR/config.ini.template`, IPs `192.168.*`, hosts/caminhos, `analise*.md`) **sanitizados** dos arquivos versionados. Histórico reescrito para **um único commit** (`646c8cb`, `git push --force`) — nenhum segredo em commit algum (`git rev-list --all` + `git grep` = 0). Templates seguros (`*.template`, `config.web.example.php`) versionados. Pendente: **rotacionar** as senhas da frota (`pdv`/`drogasil`/`drogaraia`) e a chave Fernet, que circularam antes do rewrite.
